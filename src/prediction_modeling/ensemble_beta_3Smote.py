...
# ─────────────────────────────────────────────────────────────────────────────
def apply_smoter(X_train: np.ndarray, y_train: np.ndarray,
                 n_bins: int = 4, random_state: int = 42) -> tuple:
    """
    Regression-adapted SMOTE (SMOTER-lite), applied strictly to training data.

    Strategy
    --------
    1. Bin y_train into n_bins quantile groups so SMOTE has discrete "classes"
       to balance.
    2. Run SMOTE on (X_train, bin_labels) to up-sample underrepresented titer
       ranges.  k_neighbors is clamped to min_class_count - 1 so SMOTE doesn't
       crash on small bins.
    3. For each synthetic X, recover a continuous y via inverse-distance-weighted
       interpolation from its 2 nearest real neighbours in scaled feature space.
       This mirrors how SMOTE interpolates X (convex combination of neighbours)
       and avoids assigning the bin centroid, which would introduce bias.

    Notes
    -----
    - Must be called AFTER imputation and scaling so distances are meaningful.
    - Applied only to training data; test / holdout data are never touched.
    - Falls back to the original (X_train, y_train) unchanged if any bin has
      fewer than 2 samples (SMOTE cannot operate).

    Args:
        X_train      : scaled, imputed training features  (n_train, n_features)
        y_train      : continuous titer values             (n_train,)
        n_bins       : number of quantile bins for binning y (default 4)
        random_state : RNG seed for SMOTE reproducibility

    Returns:
        X_aug  (np.ndarray) : augmented feature matrix
        y_aug  (np.ndarray) : augmented continuous target vector
    """
    try:
        from imblearn.over_sampling import SMOTE
    except ImportError:
        raise ImportError(
            "imbalanced-learn is required for SMOTE augmentation.\n"
            "Install it with:  pip install imbalanced-learn\n"
            "or:               uv add imbalanced-learn"
        )

    # ── 1. Quantile-bin y into discrete classes ───────────────────────────────
    bin_edges = np.quantile(y_train, np.linspace(0, 1, n_bins + 1))
    bin_edges[-1] += 1e-9          # ensure max value falls in the last bin
    y_bins = np.digitize(y_train, bin_edges[1:-1])   # 0-indexed class labels

    class_counts = np.bincount(y_bins)
    min_count = class_counts.min()

    if len(class_counts) < 2 or min_count < 2:
        print(
            f"    [SMOTER] Skipping fold — insufficient samples per bin "
            f"(bin counts: {class_counts.tolist()}).  Returning original data."
        )
        return X_train, y_train

    # Clamp k_neighbors so SMOTE never requests more neighbours than exist.
    k_neighbors = min(5, min_count - 1)

    # ── 2. Synthesise new samples via SMOTE on binned labels ──────────────────
    smote = SMOTE(random_state=random_state, k_neighbors=k_neighbors)
    X_aug, _ = smote.fit_resample(X_train, y_bins)

    n_orig  = len(X_train)
    n_synth = len(X_aug) - n_orig

    if n_synth == 0:
        return X_train, y_train

    # ── 3. Recover continuous y for synthetic samples via IDW interpolation ───
    nbrs = NearestNeighbors(n_neighbors=2).fit(X_train)
    distances, indices = nbrs.kneighbors(X_aug[n_orig:])

    # Guard against exact-distance-zero (identical feature vectors).
    distances = np.where(distances < 1e-10, 1e-10, distances)
    weights   = 1.0 / distances
    weights  /= weights.sum(axis=1, keepdims=True)
    y_synth   = (weights * y_train[indices]).sum(axis=1)

    y_aug = np.concatenate([y_train, y_synth])

    print(
        f"    [SMOTER] {n_orig} -> {len(X_aug)} training samples "
        f"(+{n_synth} synthetic) | bins: {class_counts.tolist()} | k={k_neighbors}"
    )
    return X_aug, y_aug
...
def do_hugging_face(df, k, feature_set_description=None, models=None,
                    holdout_merged_df=None, use_smote=False):
    """
    ...
    Args:
        ...
        use_smote        : if True, apply regression-adapted SMOTE (SMOTER-lite)
                           to each training fold after imputation and scaling.
                           Synthetic samples are NEVER added to test or holdout
                           data.  See apply_smoter() for the full strategy.
    ...
    for model_name, model in models.items():
        ...
        for f in unique_folds:
            ...
            # ── SMOTER augmentation (training fold only) ──────────────────────
            # Synthetic samples are created AFTER scaling so that distance
            # geometry in feature space is meaningful.  X_test is never touched.
            if use_smote:
                print(f"  [SMOTER] Fold {f} — augmenting training set:")
                X_train, y_train = apply_smoter(X_train, y_train, random_state=42)
...
def do_modeling(v, k, feature_set_description=None, models=None,
    ...
    Args:
        ...
        use_smote        : passed through to do_hugging_face; see apply_smoter().
...
def driver():
    ...
    Orchestrate pipeline:
      ...
      2. Run CV modeling TWICE per embedding family — once without SMOTE
         (baseline) and once with SMOTER augmentation — keeping everything else
         identical so the only variable is data augmentation.
      3. Print a head-to-head Spearman rho comparison table.
      4. Compute Spearman-weighted ensemble for both conditions.
      5. Optionally evaluate a withheld holdout set.
      6. Save timestamped CSV artefacts for both conditions.
    """
    # ── Data paths ─────────────────────────────────────────────────────────────
    PICKLE_DIR    = "../../data/modeling"
    CDR_CSV_PATH  = "../../data/modeling/cdr_features_titer.csv"
    HOLDOUT_CSV_PATH = None   # set to a CSV path to enable holdout evaluation
    ...
    # Storage for both conditions.
    conditions = {
        "baseline": {"use_smote": False, "cv_results": [], "oof": {}, "holdout_results": [], "holdout_preds": []},
        "smote":    {"use_smote": True,  "cv_results": [], "oof": {}, "holdout_results": [], "holdout_preds": []},
    }
    ...
    for condition_name, cond in conditions.items():
        ...
        for k, v in df_dict.items():
            ...
            run_results, oof_preds, holdout_results, holdout_preds = do_modeling(
                ...
                use_smote=cond["use_smote"],
            )

            # Tag each row so condition is traceable in saved CSVs.
            run_results["smote_condition"] = condition_name
            cond["cv_results"].append(run_results)

            # Prefix OOF keys so both conditions can coexist in the same dict.
            prefixed_oof = {f"{condition_name}__{key}": val for key, val in oof_preds.items()}
            cond["oof"].update(prefixed_oof)

            if not holdout_results.empty:
                holdout_results["smote_condition"] = condition_name
                cond["holdout_results"].append(holdout_results)
            if not holdout_preds.empty:
                holdout_preds["smote_condition"] = condition_name
                cond["holdout_preds"].append(holdout_preds)

    # ── Side-by-side Spearman rho comparison ──────────────────────────────────
    print("\n" + "=" * 65)
    print("SMOTE IMPACT: BASELINE vs SMOTER — Spearman rho comparison")
    print("=" * 65)

    baseline_df = pd.concat(conditions["baseline"]["cv_results"], ignore_index=True)
    smote_df    = pd.concat(conditions["smote"]["cv_results"],    ignore_index=True)

    comparison = baseline_df[["feature_set_description", "model_type", "Spearman_rho", "RMSE"]].copy()
    comparison = comparison.rename(columns={
        "Spearman_rho": "rho_baseline",
        "RMSE":         "rmse_baseline",
    })
    smote_cols = smote_df[["feature_set_description", "model_type", "Spearman_rho", "RMSE"]].rename(
        columns={"Spearman_rho": "rho_smote", "RMSE": "rmse_smote"}
    )
    comparison = comparison.merge(smote_cols, on=["feature_set_description", "model_type"], how="outer")
    comparison["rho_delta"]  = comparison["rho_smote"]  - comparison["rho_baseline"]
    comparison["rmse_delta"] = comparison["rmse_smote"] - comparison["rmse_baseline"]
    comparison = comparison.sort_values("rho_baseline", ascending=False).reset_index(drop=True)

    print(comparison[[
        "feature_set_description", "model_type",
        "rho_baseline", "rho_smote", "rho_delta",
        "rmse_baseline", "rmse_smote", "rmse_delta",
    ]].to_string(index=False))

    print("\n[NOTE] rho_delta > 0 means SMOTE improved rank correlation.")
    print("[NOTE] A large positive delta with small N is likely noise, not signal.")
    print("       Cross-validate the finding before drawing conclusions.")

    # ── Ensemble for each condition ────────────────────────────────────────────
    for condition_name, cond in conditions.items():
        if not cond["oof"]:
            continue
        print(f"\n{'=' * 55}")
        print(f"WEIGHTED ENSEMBLE — {condition_name.upper()}")
        print("=" * 55)
        try:
            ens = compute_weighted_ensemble(cond["oof"])
            print(ens["weight_table"].to_string(index=False))
            print(f"Ensemble Spearman rho : {ens['ensemble_spearman_rho']:.4f}")
            print(f"Ensemble R2           : {ens['ensemble_r2']:.4f}")
            print(f"Ensemble RMSE         : {ens['ensemble_rmse']:.4f}")
        except ValueError as e:
            print(f"  Ensemble skipped: {e}")

    # ── Failure diagnostics for baseline condition only ────────────────────────
    if conditions["baseline"]["oof"]:
        print("\n" + "=" * 55)
        print("FAILURE OVERLAP DIAGNOSTICS (baseline condition)")
        print("=" * 55)
        diag_result = compute_failure_overlap_diagnostics(
            conditions["baseline"]["oof"], high_error_pct=0.25
        )
    ...
    all_cv_results = pd.concat(
        conditions["baseline"]["cv_results"] + conditions["smote"]["cv_results"],
        ignore_index=True,
    ).sort_values(["smote_condition", "feature_set_description", "RMSE"]).reset_index(drop=True)
    all_cv_results.to_csv(f"../../data/modeling/final_results_SMOTE_{timestamp}.csv", index=False)

    comparison.to_csv(f"../../data/modeling/smote_delta_comparison_SMOTE_{timestamp}.csv", index=False)

    if conditions["baseline"]["oof"]:
        diag_result["per_antibody_df"].to_csv(
            f"../../data/modeling/failure_overlap_SMOTE_{timestamp}.csv", index=False
        )
        diag_result["error_corr_matrix"].to_csv(
            f"../../data/modeling/error_correlation_SMOTE_{timestamp}.csv"
        )

    print(f"\nSaved: final_results_SMOTE_{timestamp}.csv")
    print(f"Saved: smote_delta_comparison_SMOTE_{timestamp}.csv")
    print(f"Saved: failure_overlap_SMOTE_{timestamp}.csv")
    print(f"Saved: error_correlation_SMOTE_{timestamp}.csv")
